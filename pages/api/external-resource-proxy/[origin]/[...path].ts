...
const proxyMiddleware = (createProxyMiddleware as any)({
    ...
    onProxyRes: responseInterceptor((async(_responseBuffer, _proxyRes, req, res) => {
        const hasPrefix = (res.getHeader("location") as any)?.startsWith("/api/external-resource-proxy/")
        if (!hasPrefix) {
            res.setHeader(
                "location",
