...
import { getRateLimitMiddlewares } from "@/src/utils/rate-limiting.js"
import { callMiddlewares } from "@/src/utils/middleware.js"
import {
    getRequestType,
    getShareMessage,
...
async function sendEmail(to, from, subject, body) {
    const notificationsAPIClient = await createNotificationsAPIClient({
        baseUrl: process.env.NEXT_PUBLIC_NOTIFICATIONS_API_BASE_URL as string,
        apiKey: process.env.NEXT_PUBLIC_NOTIFICATIONS_API_KEY as string,
    })

    await notificationsAPIClient.send({
        action: "email",
        environment: process.env.NEXT_PUBLIC_SHARE_SERVICES_EMAIL_ENVIRONMENT as string,
        provider: process.env.NEXT_PUBLIC_SHARE_SERVICES_EMAIL_PROVIDER as string,
        notifications: {
            template: process.env.NEXT_PUBLIC_SHARE_SERVICES_EMAIL_TEMPLATE as string,
            notifications: [
                {
                    to: to,
                    from: from,
