...
export default function initialiseRequestInterceptor() {
    if (!process.env.NEXT_PUBLIC_PROXY_DOMAINS) {
        return
    }
    let proxyDomains
    try {
        proxyDomains = JSON.parse(process.env.NEXT_PUBLIC_PROXY_DOMAINS)
    } catch (error) {
        console.error("Could not parse NEXT_PUBLIC_PROXY_DOMAINS:", process.env.NEXT_PUBLIC_PROXY_DOMAINS)
    }
    ...
    zeroRatingHelper.replaceDomain = function(oldUrlString) {
        ...
        if (url.host in proxyDomains) {
            ...
            url = newUrl;
        } else if (url.host !== location.host && !url.host.endsWith("docker.dev")) {
