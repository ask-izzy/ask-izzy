...
import { flattenDomain } from "@/helpers/url.helpers.js"


export default function initialiseRequestInterceptor() {
    const proxyDomains = new Set(process.env.DOMAINS_TO_PROXY)
    ...
    zeroRatingHelper.replaceDomain = function(oldUrlString) {
        ...
        if (proxyDomains.has(url.host)) {
            // Set the protocol and port based on the current location to better support using the proxy in local dev
            const newOrigin = `${location.protocol}//${flattenDomain(url.host)}.` +
                process.env.NEXT_PUBLIC_PROXY_DOMAIN_SUFFIX + (location.port ? `:${location.port}` : "")
