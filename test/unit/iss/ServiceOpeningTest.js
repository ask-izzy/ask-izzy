        ...
        return new ServiceOpening(
            {
                ...
                opening_hours: openingHours.map((oh) => _.clone(oh)),
            },
            function() {return time(now);}
        );
    };
}
...
describe('ServiceOpening', function() {
    describe('open 24*7 except public holidays', function() {
        var service = serviceFactory(days.map(
            function(d) {
                return {day: d, open: "00:00:00", close: "24:00:00"};
            }
        ));

        xit('never opens or closes', function() {
            var s = service(true, "2015-09-09 1pm");
            assert.equal(s.open, true);

            assertEqualTime(s.nextCloses, time("2015-09-09 5pm"));
            assertEqualTime(
                s.nextOpeningTimes.start,
                time("2015-09-11 9am")
            );
        });
    ...
    describe('open from 9am to 5pm on Wed/Fri', function() {
        ...
        describe("when it's 7am on Wednesday", function() {
            var s = () => service(false, "2015-09-09 7am");
        ...
        describe("when it's 7pm on Wednesday", function() {
            var s = () => service(false, "2015-09-09 7pm");
