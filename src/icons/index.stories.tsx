...
for (const name of Object.keys(icons)) {
    ...
    for (const story of stories) {
        componentStories.add(story.name, args => <Component {...args} />, {
            component: Component,
            args: {
...
export function ListAllIcons(): ReactNode {
    const iconList = Object.keys(icons).map(name => ({
        name,
        component: icons[name]
    }));
    return <ul className="ListAllStories">
        {iconList.map(({
            name,
            component: Icon
        }) => <li key={name}>
            <LinkTo
                kind={`Icons/Generated/${name}`}
                story="Big"
            >
                <Icon className="big" />
                <br />
                <span>
                    {name.replace(/((?:[a-z]|[A-Z]+)(?=[A-Z]))/g, "$1​")}
                </span>
            </LinkTo>
        </li>)}
    </ul>;
