...
import type {AddressLocationProps} from "../iss/AddressLocation"
import type {
    ymdWithDashesDate,
    travelTime,
    nowOpen,
...
export default class Service {
    ...
    async getSiblingServices(): Promise<Service[]> {
        if (this.#siblingServices) {
            return this.#siblingServices;
        }

        // limit should be 0 - see
        // https://redmine.office.infoxchange.net.au/issues/112476
        let request_: SearchQuery = {
            siteId: this.site.id,
            maxPageSize: 100,
        };
        const search = createServiceSearch(request_)
        await search.loadNextPage()
        const services = search.loadedServices

        // Exclude self from services at site
        this.#siblingServices = services.filter(
