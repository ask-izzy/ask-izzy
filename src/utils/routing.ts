...
export function getPersonalisationPagesToShowFromCategory(
    ...
    const pages = getPersonalisationPagesFromCategory(category)
...
export function getPathOfSSRPage(
    ...
    return convertRouteToPath(router.pathname, router.query as Record<string, string>)
}
...
export function getServicesPath({
    ...
    if (searchText === undefined && pageMounted) {
        searchText = router.query.search && decodeURIComponent(router.query.search as string)
    }
    ...
    const pathSegments = ["", (category as Category).key]

    if ((category as Category).key === "search" && searchText) {
    ...
    if (!personalisationPage) {
        const personalisationPages =
            getPersonalisationPagesToShowFromCategory(category as Category)
                .filter(page => {
