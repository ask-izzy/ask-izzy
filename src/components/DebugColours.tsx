...
export default function DebugColours() {
    ...
    return <details style={style as any}>
...
function getColourVars(): any {
    ...
    const colourVars = Array.from(document.styleSheets)
            ...
            (acc, sheet) =>
                (acc = [
                    ...
                    ...Array.from(sheet.cssRules).reduce(
                        (def, rule) =>
                            (def =
                                (rule as any).selectorText === ":root" ?
                                    [
                                        ...def,
                                        ...Array.from((rule as any).style).filter(name =>
                                            (name as string).match(/^--(?:raw-)?colour-/),
                                        ),
                                    ] as any
                                    : def),
                        [],
        ...
        .filter(
            colourVar => !(colourVar as string).match(/-(?:hue|saturation|lightness|lightness-decimal|alpha)$/),
        )
        .filter(
